import type { FieldErrorSummary, FormAnalyticsCollection, TabSummary } from "@/types/form-analytics"
...
export function calculateAverageCompletionTime(data: FormAnalyticsCollection): number {
  const completedSessions = data.sessions.filter((session) => session.formSubmitted)
  if (completedSessions.length === 0) return 0

  const totalTime = completedSessions.reduce((sum, session) => sum + session.formCompletionTime, 0)
...
export function calculateSubmissionRate(data: FormAnalyticsCollection): number {
  ...
  const submittedCount = data.sessions.filter((session) => session.formSubmitted).length
  return (submittedCount / data.sessions.length) * 100
}

export function calculateAbandonmentRate(data: FormAnalyticsCollection): number {
  if (data.sessions.length === 0) return 0

  const abandonedCount = data.sessions.filter((session) => session.formAbandoned).length
  return (abandonedCount / data.sessions.length) * 100
}

export function getTopProblemFields(data: FormAnalyticsCollection, limit = 3): FieldErrorSummary[] {
  // Create a map to count errors by field
  const fieldErrorCounts: Record<string, { count: number; errors: Record<string, number> }> = {}
  ...
  const fieldSummaries: FieldErrorSummary[] = Object.entries(fieldErrorCounts).map(([fieldId, data]) => {
  ...
  return fieldSummaries.sort((a, b) => b.errorCount - a.errorCount).slice(0, limit)
}
...
export function getValidationErrorTypes(data: FormAnalyticsCollection): { name: string; count: number }[] {
...
function extractErrorType(errorMessage: string): string {
  ...
  if (errorMessage.includes("min") || errorMessage.includes("max")) return "Length"
